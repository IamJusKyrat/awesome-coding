...
public class NodesInASubtree {
    ...
    //TODO: Should optimize this approach with DP
    int[] countOfNodes(Node root, ArrayList<Query> queries, String s) {
        // Write your code here
        int[] results = new int[queries.size()]; int i = 0;
        //Map<Integer, List<Character, Integer>> mapping = new HashMap<>();
        for(Query query: queries) {
            //if(mapping.containsKey(query.u)) results[i++] = mapping.get(query.u);
            Node base = traverseToNode(root, query.u);
            results[i++] = findCounts(s, base, query.c);
        }

        return results;
    }

    Integer findCounts(String s, Node node, char c) {
        if(node == null) return 0;
        int localCount = 0;
        for(Node child: node.children) {
            localCount +=findCounts(s, child, c);
        }
        return s.charAt(node.val- 1) == c ? localCount+1: localCount;
    }

    Node traverseToNode(Node node, int i) {
        if(node == null) return null;
        if(i == node.val) return node;
        for(Node child: node.children) {
            Node found = traverseToNode(child, i);
            if(found != null) {
                return found;
            }
        }
